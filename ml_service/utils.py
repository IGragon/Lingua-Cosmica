...
def download_video(youtube_video_url):
    try:
        ...
        if stream and english_audio_stream:
            ...
            audio_file = english_audio_stream.download(AUDIO_STORAGE_PATH, filename=filename, filename_prefix="audio_")
            return video_file, audio_file
        else:
            return None, None
    except Exception as e:
        logger.error(f"Error downloading {youtube_video_url=}: {e}")
        return None, None
